"""Ozon Selenium block probe — same flow as CloakBrowser parser.

Run from backend/:
  uv run --with selenium scripts/ozon_selenium_block_test.py
  OZON_HEADLESS=1 uv run --with selenium scripts/ozon_selenium_block_test.py --rounds 5
"""
...
def _ozon_blocked(html: str, _driver) -> bool:
...
def _run_search_flow(
    ...
    *,
    ...
    run_async_js(driver, _geo_script())
    time.sleep(0.5)
    step = StepResult(
        label="geo_script",
    ...
    step.title = page_title(driver)
    step.captcha = captcha(driver, probe.check_captcha)
    if step.captcha:
        step.blocked = True
        steps.append(step)
        return steps, True
    ...
    goto(driver, _search_api_url(query), 3.0)
...
def _run_detail_flow(
    ...
    *,
    ...
    for idx, (label, url) in enumerate(
        ...
        goto(driver, url, 1.5 if idx == 4 else 1.0)
        step = inspect_step(
            probe,
            driver,
            label=label,
...
PROBE = SiteProbe(
    ...
    out_subdir="ozon-selenium-block",
...
if __name__ == "__main__":
    sys.exit(main(PROBE, description="Ozon Selenium block probe (mirrors CloakBrowser flow)"))
